...
public class UIBehave : MonoBehaviour {
	...
	public void OptionUsernameSave(GameObject inf) {
		HGIO.WRITE(inf.transform.Find("Text").GetComponent<Text>().text, HGIO.PATH_dasdata("userinfo.das"), FileMode.Create);
		HGIO.CLOSE();
		HGMenuPreload.ChangeUsername(inf.transform.Find("Text").GetComponent<Text>().text);
	}
